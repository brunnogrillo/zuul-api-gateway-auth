...
public class AuthorityResource extends ResourceSupport {
	...
	public AuthorityResource(Authority authority) {
		this.authority = new AuthorityResourceDTO(authority.getResource(), authority.getUser(), authority.getServer());
		...
		add(linkTo(methodOn(ServerController.class).findById(authority.getServer().getId())).withRel("server"));
